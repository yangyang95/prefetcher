...
static watch_p create_clock(void)
{
...
static void destroy(watch_p S)
{
    free(S);
}

static void start(watch_p Q)
{
    if (!(Q->running)) {
        Q->running = true;
        Q->total = (struct timespec) {
